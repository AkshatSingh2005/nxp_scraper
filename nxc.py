...
def parse_product_page(html: str, product_url: str):
    ...
    status_el = soup.find(
        ...
        and tag.get_text(strip=True).upper() in (
            "ACTIVE",
    ...
    # ----- UPDATED SHORT DESCRIPTION LOGIC -----
    # Short description – usually a marketing paragraph near the top.
    # We skip obvious UI/helper texts like "Roll over image to zoom in", etc.
    short_desc = ""
    if h1:
    ...
    # Datasheet URL – from any PDF link labelled "Data Sheet"
    doc_link = None
...
def build_rows(limit_parts: int = 6):
    ...
    for group in product_groups:
        ...
        if not pkgq_url:
            pkgq_url = product_url + "?tab=Package_Quality_Tab"
        ...
        for env in env_rows:
            ...
            grade = "Industrial"  # for MCX A assume Industrial
